...
@SuppressStaticInitializationFor({"PluginSettings", "OSGlobals"})
public class CollectMetricsTests extends AbstractTests {
    @Mock CircuitBreakerService circuitBreakerService;

    @Mock AllCircuitBreakerStats circuitBreakerStats;
    ...
    private void mockWriteValue(File file, MetricsProcessor collector) {
        Mockito.doAnswer(
                ...
                .when(collector)
                .saveMetricValues(Mockito.any(String.class), Mockito.anyLong());
    }
    ...
    public void testCircuitBreakerMetric() throws Exception {
        ...
        long estimated1 = 0;
        long tripped1 = 2;
        stats[0] = new CircuitBreakerStats(requestType, limit1, estimated1, 1, tripped1);
        long limit2 = 19607637196L;
        long estimated2 = 3;
        long tripped2 = 5;
        stats[1] = new CircuitBreakerStats(fieldDataType, limit2, estimated2, 4, tripped2);
        ...
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader(metricFile))) {
    ...
    public void testTCP() throws Exception {
        ...
        String metricFilePath = rootLocation + File.separator + PerformanceAnalyzerMetrics.sTCPPath;
        File metricFile = new File(metricFilePath);
        ...
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader(metricFile))) {
            ...
            assertEquals(
                    destTcp1.txQueueTot * 1.0 / destTcp1.numFlows,
                    parseDouble(map.get(txQ)),
                    0.001);
            assertTrue(destTcp1.numFlows == parseInt(map.get(numFlows)));
            assertEquals(
                    destTcp1.rxQueueTot * 1.0 / destTcp1.numFlows,
                    parseDouble(map.get(rxQ)),
                    0.001);
            assertEquals(
                    destTcp1.currentLostTot * 1.0 / destTcp1.numFlows,
                    parseDouble(map.get(curLost)),
                    0.001);
            assertEquals(
                    destTcp1.sendCWNDTot * 1.0 / destTcp1.numFlows,
                    parseDouble(map.get(sndCWND)),
                    0.001);
            assertEquals(
                    destTcp1.SSThreshTot * 1.0 / destTcp1.numFlows,
                    parseDouble(map.get(SSThresh)),
                    0.001);
            ...
            assertEquals(
                    destTcp2.txQueueTot * 1.0 / destTcp2.numFlows,
                    parseDouble(map.get(txQ)),
                    0.001);
            assertTrue(destTcp2.numFlows == parseInt(map.get(numFlows)));
            assertEquals(
                    destTcp2.rxQueueTot * 1.0 / destTcp2.numFlows,
                    parseDouble(map.get(rxQ)),
                    0.001);
            assertEquals(
                    destTcp2.currentLostTot * 1.0 / destTcp2.numFlows,
                    parseDouble(map.get(curLost)),
                    0.001);
            assertEquals(
                    destTcp2.sendCWNDTot * 1.0 / destTcp2.numFlows,
                    parseDouble(map.get(sndCWND)),
                    0.001);
            assertEquals(
                    destTcp2.SSThreshTot * 1.0 / destTcp2.numFlows,
                    parseDouble(map.get(SSThresh)),
                    0.001);
        }
    }
    ...
    public void testIP() throws Exception {
        ...
        String metricFilePath = rootLocation + File.separator + PerformanceAnalyzerMetrics.sIPPath;
        File metricFile = new File(metricFilePath);
        ...
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader(metricFile))) {
            ...
            Map<String, Long> currentMetrics6 = spyNetInterface.getCurrentMetrics6();
            ...
            long timeDelta =
                    NetworkInterface.getKvTimestamp() - NetworkInterface.getOldkvTimestamp();
            double inbps = 8 * 1.0e3 * (curphy.get("inbytes") - oldphy.get("inbytes")) / timeDelta;
            double outbps =
                    8 * 1.0e3 * (curphy.get("outbytes") - oldphy.get("outbytes")) / timeDelta;
            ...
            assertEquals(inPacketRate4, parseDouble(map.get(packetRate4)), 0.001);
            assertEquals(inDropRate4, parseDouble(map.get(dropRate4)), 0.001);
            assertEquals(inPacketRate6, parseDouble(map.get(packetRate6)), 0.001);
            ...
            assertEquals(outPacketRate4, parseDouble(map.get(packetRate4)), 0.001);
    ...
    private static List<DiscoveryNode> buildNumNodes(
        ...
        final DiscoveryNode node2 =
                new DiscoveryNode(
                        "Zn1QcSU",
                        nodeId2,
                        new TransportAddress(address2, 9200),
                        emptyMap(),
    ...
    public void testNodeDetails() throws Exception {
        ...
        String metricFilePath =
                rootLocation + File.separator + PerformanceAnalyzerMetrics.sNodesPath;
        File metricFile = new File(metricFilePath);
